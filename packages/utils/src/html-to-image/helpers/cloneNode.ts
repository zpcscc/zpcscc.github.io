...
async function cloneCanvasElement(node: HTMLCanvasElement) {
  ...
  if (dataURL === 'data:,') {
    return await Promise.resolve(node.cloneNode(false) as HTMLCanvasElement);
  }

  return createImage(dataURL);
}

async function cloneVideoElement(node: HTMLVideoElement, options: OptionsType) {
  return await Promise.resolve(node.poster)
...
async function cloneSingleNode<T extends HTMLElement>(
  ...
  return await Promise.resolve(node.cloneNode(false) as T);
}
